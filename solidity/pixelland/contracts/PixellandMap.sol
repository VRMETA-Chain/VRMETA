...
contract PixellandMap is Ownable {
    ...
    function getMapSize() public view returns(uint[2] memory) {
        return mapDimensions;
    }

    //returns the most recent proposed coordinates + proposer in array[0].
    function getRecentProposal() public view returns(Plot memory){
        return propList[0];
    }

    function getProposalByAddress(address addr) public view returns(uint[2][2] memory) {
        return plotProposal[addr].coords;
    }
